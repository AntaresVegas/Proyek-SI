...
$background_path = '../img/backgroundSekretariat.jpg'; 
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Registrasi Akun Sekretariat - Sistem Event Unpar</title>
...
<body>
  <main class="container">
    <div class="header">
        <img src="../img/logoSU.png" alt="Logo Sekretariat Unpar">
        <h1>Registrasi Akun Sekretariat</h1>
        <p>Lengkapi data Anda untuk melanjutkan.</p>
    </div>

    <?php if (isset($_SESSION['error_sekretariat'])) : ?>
      <div class="message-box error-message">
        <?php echo htmlspecialchars($_SESSION['error_sekretariat']); ?>
      </div>
      <?php unset($_SESSION['error_sekretariat']); ?>
    <?php endif; ?>

    <form action="send_otp_sekretariat.php" method="POST" autocomplete="off">
        <div class="form-group">
            <label for="sekuniv_nama">Nama Lengkap</label>
            <input type="text" id="sekuniv_nama" name="sekuniv_nama" required value="<?php echo isset($_SESSION['old_data_sekretariat']['nama']) ? htmlspecialchars($_SESSION['old_data_sekretariat']['nama']) : ''; ?>" placeholder="Masukkan nama sesuai data karyawan">
            <i class="input-icon fa-solid fa-user"></i>
        </div>
        <div class="form-group">
            <label for="sekuniv_NIK">NIK (Nomor Induk Karyawan)</label>
            <input type="text" id="sekuniv_NIK" name="sekuniv_NIK" required value="<?php echo isset($_SESSION['old_data_sekretariat']['nik']) ? htmlspecialchars($_SESSION['old_data_sekretariat']['nik']) : ''; ?>" pattern="\d*" placeholder="Masukkan NIK Anda">
            <i class="input-icon fa-solid fa-id-card"></i>
        </div>
        <div class="form-group">
            <label for="sekuniv_email">Email</label>
            <input type="email" id="sekuniv_email" name="sekuniv_email" required value="<?php echo isset($_SESSION['old_data_sekretariat']['email']) ? htmlspecialchars($_SESSION['old_data_sekretariat']['email']) : ''; ?>" placeholder="Gunakan email Unpar">
            <i class="input-icon fa-solid fa-envelope"></i>
        </div>
        <?php unset($_SESSION['old_data_sekretariat']); ?>
