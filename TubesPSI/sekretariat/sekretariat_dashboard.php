...
// Check if user is logged in and is Sekretariat
...
try {
    if (isset($conn) && $conn->ping()) {
        // [LOGIKA SEKRETARIAT] Count total FINAL APPROVED events starting this month
        $stmt_events_month = $conn->prepare("SELECT COUNT(*) as total FROM pengajuan_event WHERE MONTH(pengajuan_event_tanggal_mulai) = ? AND YEAR(pengajuan_event_tanggal_mulai) = ? AND pengajuan_status_proposal = 'Disetujui'");
        ...
        // [LOGIKA SEKRETARIAT] Count completed FINAL APPROVED events this month
        $stmt_completed_month = $conn->prepare("SELECT COUNT(*) as total FROM pengajuan_event WHERE MONTH(pengajuan_event_tanggal_selesai) = ? AND YEAR(pengajuan_event_tanggal_selesai) = ? AND pengajuan_event_tanggal_selesai < ? AND pengajuan_status_proposal = 'Disetujui'");
        ...
        // [LOGIKA SEKRETARIAT] Count ALL pending event approvals (still in process)
        $result_pending = $conn->query("SELECT COUNT(*) as total FROM pengajuan_event WHERE pengajuan_status_proposal = 'Diajukan'");
        if ($result_pending) {
            $pending_approvals = $result_pending->fetch_assoc()['total'];
        }

        // [LOGIKA SEKRETARIAT] Count rejected events this month (ditolak oleh Ditmawa ATAU ASP)
        $stmt_rejected_month = $conn->prepare("SELECT COUNT(*) as total FROM pengajuan_event WHERE MONTH(pengajuan_event_tanggal_mulai) = ? AND YEAR(pengajuan_event_tanggal_mulai) = ? AND (pengajuan_status_ditmawa = 'Ditolak' OR pengajuan_status_asp = 'Ditolak' OR pengajuan_status_proposal = 'Ditolak')");
        ...
        // [LOGIKA SEKRETARIAT] Fetch calendar events based on FINAL approval status
        $stmt_calendar_events = $conn->prepare("
            ...
            WHERE pe.pengajuan_status_proposal = 'Disetujui'
            AND (
            ...
            while ($row = $result_calendar_events->fetch_assoc()) {
                ...
                foreach ($period as $day) {
                    if ($day->format('n') == $currentMonth && $day->format('Y') == $currentYear) {
                        ...
                        $calendar_events[$day_num][] = ['name' => htmlspecialchars($row['pengajuan_namaEvent']), 'type' => 'main'];
                    }
                }
                    ...
                    if ($prep_start_dt < $main_event_start_dt) {
                        ...
                        foreach ($prep_period as $dt) {
                             if ($dt->format('n') == $currentMonth && $dt->format('Y') == $currentYear) {
                                ...
                                $calendar_events[$day_num][] = ['name' => htmlspecialchars($row['pengajuan_namaEvent']) . ' (Persiapan)', 'type' => 'prep'];
                             }
                        }
                    }
                }
                    ...
                    $clear_end_dt = new DateTime($row['tanggal_beres']);
                     if ($clear_end_dt > $main_event_end_dt) {
                        ...
                        foreach ($clear_period as $dt) {
                             if ($dt->format('n') == $currentMonth && $dt->format('Y') == $currentYear) {
                                $day_num = (int)$dt->format('j');
                                $calendar_events[$day_num][] = ['name' => htmlspecialchars($row['pengajuan_namaEvent']) . ' (Pembongkaran)', 'type' => 'clear'];
                            }
                        }
                    }
                }
    ...
    error_log("Error in Sekretariat Dashboard: " . $e->getMessage());
}
$conn->close();
?>
    ...
    <head>
        ...
        <title>Dashboard Sekretariat - Event Management Unpar</title>
        ...
        <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            html { height: 100%; }
            /* [DIUBAH] Menggunakan background Sekretariat */
            body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-image: url('../img/backgroundSekretariat.jpg'); background-size: cover; background-position: center center; background-repeat: no-repeat; background-attachment: fixed; min-height: 100vh; padding-top: 80px; display: flex; flex-direction: column; }
            ...
            .welcome-section { background: linear-gradient(135deg, var(--secondary-color) 0%, var(--primary-color) 100%); color: white; border-radius: 10px; padding: 25px; margin-bottom: 30px; text-align: center; }
            ...
            .stat-card.events { border-color: var(--primary-color); }
            ...
            .stat-card.events .icon { color: var(--primary-color); }
            ...
            .calendar-header h2 { font-size: 1.5em; color: var(--secondary-color); }
            .calendar-header a { text-decoration: none; color: var(--primary-color); font-size: 1.5em; transition: color 0.3s; }
            .calendar-header a:hover { color: var(--secondary-color); }
            .calendar-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 5px; }
            .day-name { text-align: center; font-weight: 600; padding: 10px 0; font-size: 0.9em; color: #34495e; }
            ...
            .day-cell.today .day-number { background-color: var(--primary-color); color: white; border-radius: 50%; width: 28px; height: 28px; display: inline-flex; align-items: center; justify-content: center; }
            .event-indicator { font-size: 0.9em; padding: 3px 6px; border-radius: 4px; margin-top: 3px; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
            .event-indicator.main { background-color: #E3F2FD; color: #0D47A1; } /* Light Blue */
            .event-indicator.prep, .event-indicator.clear { background-color: #E8EAF6; color: #3F51B5; } /* Light Indigo */
            ...
            .detail-link-container { text-align: center; margin-top: 20px; }
            .detail-link { color: var(--primary-color); text-decoration: none; font-weight: bold; }
    ...
    <nav class="navbar">
        ...
        <ul class="navbar-menu">
            <li><a href="sekretariat_dashboard.php" class="active">Home</a></li>
        ...
        <div class="container">
            <div class="content-card">
                <div class="welcome-section">
                    <h2>Selamat Datang di Dashboard Sekretariat</h2>
                    <p>Ringkasan dan kalender event Universitas Katolik Parahyangan</p>
                </div>
                <div class="stats-grid">
                    <div class="stat-card events"><div class="icon"><i class="fas fa-calendar-day"></i></div><div class="number"><?php echo $total_events_this_month; ?></div><div class="label">Event Disetujui Bulan Ini</div></div>
                    ...
                    <div class="stat-card pending"><div class="icon"><i class="fas fa-hourglass-half"></i></div><div class="number"><?php echo $pending_approvals; ?></div><div class="label">Event Dalam Proses</div></div>
            ...
            <div class="content-card">
                <div class="calendar-wrapper">
                    <h2 class="calendar-title">KALENDER EVENT UNIVERSITAS</h2>
                        ...
                        ?>
                            <div class="<?php echo $cellClass; ?>">
                                ...
                                <?php if (isset($calendar_events[$day])):
                                    ...
                                    foreach (array_slice($unique_events, 0, 3) as $event):
                                    ?>
                                        <span class="event-indicator <?php echo $event['type']; ?>" title="<?php echo htmlspecialchars($event['name']); ?>"><?php echo htmlspecialchars($event['name']); ?></span>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </div>
                        <?php endfor; ?>
                    ...
                    <div class="calendar-legend">
                        <div class="legend-item"><span class="legend-color-box" style="background-color: #E3F2FD;"></span> Event Utama</div>
                        <div class="legend-item"><span class="legend-color-box" style="background-color: #E8EAF6;"></span> Persiapan / Pembongkaran</div>
                    </div>
                    <div class="detail-link-container">
                        <a href="sekretariat_kalender.php" class="detail-link">Klik Untuk Kalender Lebih Detail</a>
                    </div>
                </div>
            </div>
        </div>
