...
// [DIUBAH] Cek session registrasi sekretariat
if (!isset($_SESSION['reg_data_sekretariat'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // [DIUBAH] Cek session OTP sekretariat
    $otp_session = $_SESSION['reg_otp_sekretariat'] ?? null;
    $otp_expiry = $_SESSION['otp_expiry_sekretariat'] ?? 0;
    ...
    if (empty($errors)) {
        // [DIUBAH] Ambil data dari session sekretariat
        $reg_data = $_SESSION['reg_data_sekretariat'];
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);

        // [DIUBAH] Insert ke tabel sekretariat_universitas
        $stmt = $conn->prepare("INSERT INTO sekretariat_universitas (sekuniv_nama, sekuniv_email, sekuniv_password, sekuniv_NIK) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $reg_data['nama'], $reg_data['email'], $hashed_password, $reg_data['nik']);

        if ($stmt->execute()) {
            // [DIUBAH] Hapus session registrasi sekretariat
            unset($_SESSION['reg_data_sekretariat'], $_SESSION['reg_otp_sekretariat'], $_SESSION['otp_expiry_sekretariat']);
            $_SESSION['success_message'] = "Registrasi Akun Sekretariat berhasil! Silakan login.";
...
// [DIUBAH] Path background sekretariat
...
<title>Verifikasi Akun Sekretariat - Sistem Event Unpar</title>
...
<body>
  <main class="container">
    <div class="header">
        ...
        <h1>Verifikasi Akun Sekretariat</h1>
        <p>Kode verifikasi telah dikirim ke <strong><?php echo htmlspecialchars($_SESSION['reg_data_sekretariat']['email']); ?></strong></p>
    </div>
