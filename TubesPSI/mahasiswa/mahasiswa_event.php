...
$nama = $_SESSION['nama'] ?? 'Mahasiswa'; // Menggunakan 'Mahasiswa' sebagai default nama
$user_id_mahasiswa = $_SESSION['user_id'] ?? null; // ID mahasiswa yang login
...
$firstDayOfWeek = $date->format('N'); // 1 for Monday, 7 for Sunday (PHP's date('N'))
...
try {
    if (isset($conn) && $conn->ping()) {
        // Fetch event data for the current month (only 'Disetujui' events)
        ...
        if ($stmt_initial_events) {
            ...
            error_log("Failed to prepare statement for fetching initial events in mahasiswa_event.php: " . $conn->error);
        }
        ...
        if ($stmt_gedung) {
            ...
            error_log("Failed to prepare statement for fetching buildings in mahasiswa_event.php: " . $conn->error);
        }
        ...
        if ($stmt_lantai) {
            ...
            error_log("Failed to prepare statement for fetching floors in mahasiswa_event.php: " . $conn->error);
        }

    } else {
        error_log("Database connection not established or lost in mahasiswa_event.php");
    }
} catch (Exception $e) {
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Navbar styles - Disesuaikan untuk Mahasiswa (warna hijau) */
        .navbar {
            display: flex;
            justify-content: space-between;
            align-items: center;
            background:rgb(2, 71, 25); /* Warna hijau untuk mahasiswa */
            width: 100%;
            padding: 10px 30px;
        ...
        .navbar-menu li a {
            ...
            color:rgb(253, 253, 253); /* Warna teks putih */
            font-weight: 500;
            font-size: 15px;
        ...
        .icon:hover {
            color: #a1c4fd; /* Warna hover untuk ikon */
        }
        ...
        .calendar-header .nav-arrow {
            font-size: 24px;
            color: rgb(2, 71, 25); /* Warna panah sesuai tema mahasiswa */
            cursor: pointer;
        ...
        .calendar-header .nav-arrow:hover {
            color: #a1c4fd; /* Warna hover panah */
        }
        ...
        .event-indicator {
            font-size: 12px;
            color: rgb(2, 71, 25); /* Warna indikator event sesuai tema mahasiswa */
            font-weight: 600;
            margin-top: 5px;
            white-space: nowrap;
            overflow: hidden;
        ...
        .modal-body .event-item {
            background-color: #e6f7ed; /* Light green background for event items */
            border: 1px solid #c8e6c9;
            border-left: 5px solid rgb(2, 71, 25); /* Green left border */
            padding: 15px;
            margin-bottom: 15px;
            border-radius: 8px;
            display: flex;
        ...
        .modal-body .event-item h4 {
            font-size: 18px;
            color: rgb(2, 71, 25); /* Green title for event items */
            margin-bottom: 5px;
        }
        ...
        .welcome-section {
            background: linear-gradient(135deg,rgb(2, 73, 43) 0%, rgb(2, 71, 25) 100%); /* Green gradient */
            color: white;
            padding: 25px;
            margin-bottom: 30px;
            text-align: center;
...
<nav class="navbar">
    ...
    <ul class="navbar-menu">
        ...
        <li><a href="mahasiswa_event.php" class="active">Event</a></li> <li><a href="mahasiswa_laporan.php">Laporan</a></li>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchEventsAndRenderCalendar() {
            ...
            fetch(`fetch_event_data.php?month=${currentMonth}&year=${currentYear}&gedung_id=${selectedGedung}&lantai_id=${selectedLantai}&user_type=mahasiswa`) // Added user_type
        ...
        function addDayCellClickListeners() {
            ...
            dayCells.forEach(cell => {
                cell.addEventListener('click', function() {
                    ...
                    fetch(`fetch_event_details.php?date=${date}&gedung_id=${selectedGedung}&lantai_id=${selectedLantai}&user_type=mahasiswa`) // Added user_type
                        .then(response => response.json())
                        .then(data => {
