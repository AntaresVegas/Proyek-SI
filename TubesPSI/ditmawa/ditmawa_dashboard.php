...
try {
    if (isset($conn) && $conn->ping()) {
        ...
        // [FIX] Count pending event approvals based on ditmawa status
        $result_pending = $conn->query("SELECT COUNT(*) as total FROM pengajuan_event WHERE pengajuan_status_ditmawa = 'Diajukan'");
        if ($result_pending) {
        ...
        // [FIX] Fetch calendar events based on ditmawa status
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            html { height: 100%; }
            body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-image: url('../img/backgroundDitmawa.jpeg'); background-size: cover; background-position: center center; background-repeat: no-repeat; background-attachment: fixed; min-height: 100vh; padding-top: 80px; display: flex; flex-direction: column; }
            ...
            .content-card { background: rgba(255, 255, 255); border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); padding: 30px; margin-bottom: 30px; }
            ...
            .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 20px; } /* [DIUBAH] minmax */
            ...
            .stat-card.rejected { border-color: #e74c3c; } /* [BARU] */
            ...
            .stat-card.rejected .icon { color: #e74c3c; } /* [BARU] */
            ...
            .day-cell.today .day-number { background-color: #ff8c00; color: white; border-radius: 50%; width: 28px; height: 28px; display: inline-flex; align-items: center; justify-content: center; }
            ...
            .event-indicator.prep-clear { background-color: #dc3545; color: white; }
            ...
            <div class="content-card">
                <div class="calendar-wrapper">
                        ...
                        ?>
                            <div class="<?php echo $cellClass; ?>">
                                ...
                                <?php if (isset($calendar_events[$day])): 
                                    ...
                                    foreach (array_slice($unique_events, 0, 3) as $event): // Limit to 3 events on dashboard
                                        $eventTypeClass = ($event['type'] === 'main') ? '' : 'prep-clear';
                                    ?>
                                        <span class="event-indicator <?php echo $eventTypeClass; ?>" title="<?php echo htmlspecialchars($event['name']); ?>"><?php echo htmlspecialchars($event['name']); ?></span>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </div>
                        <?php endfor; ?>
                    ...
                    <div class="calendar-legend">
                        <div class="legend-item"><span class="legend-color-box" style="background-color: #ffe8cc;"></span> Event Utama</div>
                        <div class="legend-item"><span class="legend-color-box" style="background-color: #dc3545;"></span> Persiapan / Pembongkaran</div>
                    </div>
